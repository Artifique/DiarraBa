...
export default function ReservationsPage() {
  ...
  const [selectedRes, setSelectedRes] = useState<typeof initialReservations[0] | null>(null);
  const [isOpen, setIsOpen] = useState(false);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<string | null>(null);

  const [newRes, setNewRes] = useState({ client: "", items: "", qte: "", pu: "", lieu: "" });
  ...
  const handleAdd = () => {
    ...
    const item = {
      ...
      client: newRes.client === "c1" ? "Ahmed Bennani" : newRes.client === "c2" ? "Fatima Alaoui" : "Mohammed Karim",
      items: `${newRes.qte} ${newRes.items || 'Articles'}`,
      total: totalCalc,
      paye: 0,
      statut: "EnAttente" as const,
      date: new Date().toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }),
      details: { tel: "+212 ...", lieu: newRes.lieu || "Non spécifié", type: "Tranche" as const }
    };
  ...
  const handleStatusChange = (id: string, newStatus: string) => {
    setReservations(reservations.map(r => r.id === id ? { ...r, statut: newStatus as any } : r));
    ...
    <div className="h-full flex flex-col gap-6 relative overflow-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gérez les commandes clients, les acomptes et les plannings de livraison.</p>
        </div>
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="bg-night border-white/10 text-white sm:max-w-2xl overflow-hidden">
            <DialogHeader>
              <DialogTitle className="text-white font-display text-xl">Nouvelle Réservation</DialogTitle>
            ...
            <div className="grid gap-6 py-4 max-h-[70vh] overflow-y-auto px-1 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select onValueChange={(val) => setNewRes({...newRes, client: val})}>
                ...
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-widest font-bold text-white/70">Type de Paiement</Label>
                  <Select>
                    <SelectTrigger className="bg-white/5 border-white/10 text-white">
              ...
              <div className="space-y-4">
                <Label className="text-xs uppercase tracking-widest font-bold text-white/70">Articles & Tarification</Label>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                  <div className="md:col-span-2">
                    <Select onValueChange={(val) => setNewRes({...newRes, items: val})}>
                      <SelectTrigger className="bg-white/5 border-white/10 text-white">
                  ...
                  <Input 
                    type="number" 
                    value={newRes.qte}
                    onChange={(e) => setNewRes({...newRes, qte: e.target.value})}
                  ...
                  <Input 
                    type="number" 
                    value={newRes.pu}
                    onChange={(e) => setNewRes({...newRes, pu: e.target.value})}
                    className="bg-white/5 border-white/10 text-white" 
                ...
                {newRes.qte && newRes.pu && (
                  <p className="text-right text-xs font-bold text-orange-accent">
                    Total estimé : {(parseInt(newRes.qte) * parseInt(newRes.pu)).toLocaleString()} FCFA
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-widest font-bold text-white/70">Date de Livraison prévue</Label>
                  <Input type="date" className="bg-white/5 border-white/10 text-white" />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs uppercase tracking-widest font-bold text-white/70">Lieu de Livraison</Label>
                  <Input 
                    value={newRes.lieu}
                    onChange={(e) => setNewRes({...newRes, lieu: e.target.value})}
                    className="bg-white/5 border-white/10 text-white" 
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        ...
                        res.statut === "Confirmee" ? "bg-blue-400/10 text-blue-400" :
                        res.statut === "Livree" ? "bg-forest-green/10 text-forest-green" :
                        "bg-orange-accent/10 text-orange-accent"
                      )}>
                        <CalendarCheck className="h-5 w-5" />
                      </div>
                      <div>
                        <h4 className="font-bold text-white text-sm">{res.client}</h4>
                        <p className="text-[10px] text-muted-foreground uppercase font-medium">{res.id}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-mono font-bold text-white">{res.total.toLocaleString()} FCFA</p>
                      <p className="text-[10px] text-muted-foreground">{res.date}</p>
                    </div>
                  </div>
                  
                  <div className="mt-3 flex items-center justify-between">
                    <p className="text-[11px] text-muted-foreground truncate max-w-[200px] italic">"{res.items}"</p>
                    <div className={cn(
                      "px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest",
                      res.statut === "Confirmee" ? "bg-blue-400/10 text-blue-400 border border-blue-400/20" :
                      res.statut === "Livree" ? "bg-forest-green/10 text-forest-green border border-forest-green/20" :
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                ...
                <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-white">{selectedRes.client}</h4>
                      <p className="text-sm text-muted-foreground">{selectedRes.details.tel}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {selectedRes.items.split(', ').map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-white/5">
                        ...
                        <span className="text-xs text-orange-accent font-bold">Inclus</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-xs font-mono font-bold text-white">
                      {selectedRes.total > 0 ? Math.round((selectedRes.paye / selectedRes.total) * 100) : 0}% Payé
                    </span>
                  </div>
                  <div className="h-3 w-full bg-white/5 rounded-full overflow-hidden border border-white/10">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${selectedRes.total > 0 ? (selectedRes.paye / selectedRes.total) * 100 : 0}%` }}
                      transition={{ duration: 1, ease: "easeOut" }}
                      className="h-full bg-forest-green shadow-[0_0_10px_rgba(45,106,45,0.5)]" 
                    />
                  </div>
                  <div className="flex justify-between text-[11px]">
                    <span className="text-muted-foreground italic">Payé: {selectedRes.paye.toLocaleString()} FCFA</span>
                    <span className="text-white font-bold">Total: {selectedRes.total.toLocaleString()} FCFA</span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[1px] before:bg-white/10">
                    {[
                      { label: "Créée", date: selectedRes.date, active: true, icon: CheckCircle2 },
                      { label: "Confirmée", date: "En cours", active: selectedRes.statut !== "EnAttente", icon: CheckCircle2 },
                      { label: "Livrée", date: "Prévue", active: selectedRes.statut === "Livree", icon: Clock },
                    ].map((step, idx) => (
                      <div key={idx} className="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground/60">{step.date}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent className="bg-night border-white/10 text-white sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              Voulez-vous vraiment supprimer cette réservation ? Toutes les données associées seront effacées.
            </DialogDescription>
          </DialogHeader>
